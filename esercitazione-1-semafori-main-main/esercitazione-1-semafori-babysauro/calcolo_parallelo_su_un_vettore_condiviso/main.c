...
int main()
{
    ...
    vett_id = shmget(IPC_PRIVATE,sizeof(int)*1000,IPC_CREAT|0664);    /* TBD: usare shmget() per creare un vettore di interi su
    ...
    *buffer = INT_MAX;
     ...
     *      - l'indice del primo elemento da elaborare (PER ORA LO CHIAMO 0)
    ...
    for (int i=0; i<10; i++)
    {
        ...
        if (pid == 0)
        {
          figlio(vettore, buffer, sem_id, primo_elemento, 1000); 
          exit(0); 
        }
        else if (pid < 0)
        {
